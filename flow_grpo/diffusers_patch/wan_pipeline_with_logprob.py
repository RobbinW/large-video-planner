...
def wan_pipeline_with_logprob(
    ...
    Args:
        ...
        clip_fea (`torch.Tensor`, *optional*):
            CLIP features for image-to-video mode.
    ...
    elif hasattr(self.transformer.config, "in_dim"):
        num_channels_latents = self.transformer.config.in_dim
    else:
        num_channels_latents = self.transformer.config["in_dim"]


    latents = self.prepare_latents(   ##这里要看调用的哪里的函数啊。返回的latent 维度倒是对的。就是需要从Image 来编码
